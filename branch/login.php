...
if (isset($_POST["Employee_email"]) && isset($_POST["Employee_password"])) {
    $Admin_name = $_POST["Employee_email"];
    $Admin_password = $_POST["Employee_password"];
    $result = mysqli_query($con, "SELECT * FROM `employee` WHERE `Employee_email`='{$Admin_name}' AND `Employee_password`='{$Admin_password}' and Employee_type_id = '1' ")or die(mysqli_error($con));
    if (mysqli_num_rows($result) == 1) {
        $row = mysqli_fetch_assoc($result);
        session_start();
        $_SESSION["Admin_id"] = $row["Employee_id"];
        $_SESSION["Admin_name"] = $row["Employee_name"];
                                        ...
                                        <div class="form-wrap">
                                            <form method="POST">
                                                <div class="form-group">
                                                    ...
                                                    <input type="email" name="Employee_email" class="form-control" required="" id="exampleInputEmail_2" placeholder="Enter email">
                                                </div>
                                                <div class="form-group">
                                                    ...
                                                    <div class="clearfix"></div>
                                                    <input name="Employee_password" type="password" class="form-control" required="" id="exampleInputpwd_2" placeholder="Enter pwd">
                                                </div>
