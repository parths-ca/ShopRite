...
if(isset($_GET["eid"]) && !empty($_GET["eid"])){
        $eid = $_GET["eid"];
        $result = mysqli_query($con, "Select * from order_details where Order_details_id='{$eid}'")or die(mysqli_error($con));
...
if($_POST){
    ...
    $result = mysqli_query($con, "UPDATE `order_details` SET `Order_id`='{$txtOrderId}',`Product_id`='{$txtProductId}',`Quantity`='{$txtQuantity}',`Price`='{$txtOrderPrice}' WHERE Order_details_id='{$eid}' ")or die(mysqli_error($con));
    if($result){
        header("location:view-order-details.php");    }  
}
                                                ...
                                                <div class="form-wrap">
                                                    <form data-toggle="validator" method="POST" role="form">
                                                        ...
                                                        <div class="form-group">
                                                            ...
                                                            <input value="<?php echo $row["Quantity"]; ?>" type="number" class="form-control" id="inputName" name="txtQuantity" required>
                                                        </div>
                                                        <div class="form-group">
                                                            <label for="inputName" class="control-label mb-10">Order Price</label>
                                                            <input value="<?php echo $row["Price"]; ?>" type="number" class="form-control" id="inputName" name="txtOrderPrice" required>
                                                        </div>
